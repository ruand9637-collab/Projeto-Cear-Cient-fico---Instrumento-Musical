...
import pickle
import time
from collections import Counter # Otimização de contagem em tempo real
...
# Segunda alteração padrão=2
PROCESSAR_A_CADA = 3   # Processa 1 frame a cada N — aumente se ainda lagar

# Configuração camera
CONTRASTE_CAMERA = 1.4  # aumente se a webcam for muito escura/baixo contraste
BRILHO_CAMERA    = 30   # aumente se a imagem ficar muito escura
# =========================================================================
...
def melhorar_imagem(frame):
    # Aumenta contraste e brilho para compensar webcam de baixa qualidade
    # alpha = contraste (1.0 = original, 1.5 = 50% mais contraste)
    # beta  = brilho    (0 = original, 40 = mais claro)
    alpha = 1.4
    beta  = 30
    return cv2.convertScaleAbs(frame, alpha=CONTRASTE_CAMERA, beta=BRILHO_CAMERA)
...
# OTIMIZAÇÃO: Cria o contador em memória baseado no que já existe no banco
contagem_por_nota = Counter(d["label"] for d in dados_coletados)
...
while cap.isOpened():
    ...
    if frame_contador % PROCESSAR_A_CADA == 0:
        frame_melhorado = melhorar_imagem(frame)  # aplica contraste/brilho
    ...
    # OTIMIZAÇÃO: Apenas consulta o dicionário (Tempo constante O(1)), sem vasculhar a lista de 70k itens
    total_nota  = contagem_por_nota[nota_atual]
    agora       = time.time()
        ...
        if agora - ultimo_capture >= INTERVALO_AUTO:
            if amostras_auto < LIMITE_AUTO:
                for pontos_mao in resultado.hand_landmarks:
                    ...
                    # OTIMIZAÇÃO: Incrementa o contador em memória instantaneamente
                    contagem_por_nota[nota_atual] += 1
    ...
    # LÓGICA DE TECLADO CORRIGIDA (IF/ELIF) PARA EVITAR CONFLITOS DE CAPTURA
    ...
    # T — treinar agora
        ...
        with open(ARQUIVO_DADOS, "w") as f:
            json.dump(dados_coletados, f)
        print("Iniciando treinamento pesado (isso pode demorar alguns segundos devido a quantidade de amostras)...")
        treinar()
